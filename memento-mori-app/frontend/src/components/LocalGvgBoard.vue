<template>
  <div>
    <div class="toolbar">
      <div class="hint">World: {{ worldId }} <span v-if="updatedAt">/ Updated: {{ updatedAt?.toLocaleString() }}</span></div>
      <div class="legend"><span class="pill idle">Idle</span><span class="pill battle">Battle</span><span class="pill fallen">Fallen</span><span class="pill counter">Counter</span><span class="pill counterok">Counter OK</span></div>
      <div class="right">
    ...
    <div v-else class="grid">
      <div v-for="c in castles" :key="c.CastleId" class="cell" :class="stateClass(c.GvgCastleState)">
        <header>
...
function parseMessage(buf: ArrayBuffer) {
  ...
  while (off + 4 <= view.byteLength) {
    ...
    if (castle === 0) {
      ...
      const next = { CastleId: castle, GuildId: holder, AttackerGuildId: attacker, UtcFallenTimeStamp: fallen,
                     DefensePartyCount: def, AttackPartyCount: atk, GvgCastleState: state, KoCount: ko,
...
function openWs() {
  ...
  sock.onopen = () => {
    ...
    // subscribe all castles for the given world (block=0, group=0, class=0)
...
function closeWs() { if (sock) { try { sock.close() } catch {} finally { sock = null } } }
    ...
    <div v-else class="grid">
      <div v-for="c in castles" :key="c.CastleId" class="cell" :class="stateClass(c.GvgCastleState)">
        ...
        <div class="row counts">
          <div>DEF: {{ c.DefensePartyCount }}</div>
          <div>ATK: {{ c.AttackPartyCount }}</div>
          <div>KO: {{ c.KoCount ?? c.KOCount ?? 0 }}</div>
        </div>
        <footer>{{ stateLabel(c.GvgCastleState) }}</footer>
      </div>
    </div>
...
function castleName(id: number) { return castleNames[id] ?? `Castle ${id}` }
function guildName(id: number) { return id ? data.value?.data?.guilds?.[String(id)] : '' }
function stateLabel(s: number) { return ['Idle','Battle','Fallen','Counter','Counter OK'][s] ?? 'Unknown' }
function stateClass(s: number) { return ['idle','battle','fallen','counter','counterok'][s] ?? 'idle' }
...
async function load() {
  if (props.worldId == null) return
  loading.value = true
  try {
    const res = await getLocalGvg(props.worldId)
    data.value = res
    updatedAt.value = new Date((res?.timestamp || 0) * 1000)
    castles.value = (res?.data?.castles || []).map((c: any) => ({
      CastleId: c.CastleId,
      GuildId: c.GuildId,
      AttackerGuildId: c.AttackerGuildId,
      DefensePartyCount: c.DefensePartyCount ?? c.DefenseCount ?? 0,
      AttackPartyCount: c.AttackPartyCount ?? c.AttackCount ?? 0,
      GvgCastleState: c.GvgCastleState ?? c.state ?? 0,
      KoCount: c.KoCount ?? c.ko_count ?? 0,
    }))
  } finally {
    loading.value = false
  }
...
function startAutoRefresh() {
  stopAutoRefresh()
  // 20:45~21:30（各サーバ時間）は 3 秒、それ以外は 30 秒で更新
  const now = new Date()
  const sec = autoInterval(now)
  timer = setInterval(load, sec * 1000)
}

function autoInterval(now: Date) {
  // シンプルな固定：3s（戦争時間を想定）
  // 本番ではサーバ時間に合わせて切り替えを実装してもOK
  return 5
}
...
watch(() => props.worldId, () => { load(); startAutoRefresh() })
</script>
